...
local saveNote_noteCurSkinPlayer      = ternary(nil, getSave('noteCurSkinPlayer'), false)
local saveNote_noteCurSkinOpponent    = ternary(nil, getSave('noteCurSkinOpponent'), false)
...
local function setUpNoteSkins() -- me praying to god that it doesn't lag the script becuase of this
     ...
     if saveNote_noteCurSkinPlayer ~= false then 
          initNoteRGB(1, saveNote_noteCheckPlayerIndex)
     end
     if saveNote_noteCurSkinOpponent ~= false then 
          initNoteRGB(2, saveNote_noteCheckOpponentIndex)
     end
end

function onCreatePost()
     if songName ~= 'NoteSkin Settings' or songName ~= 'NoteSkin Offsetting' then
